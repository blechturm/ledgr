...
durable artifacts: sealed snapshot hash, strategy source hash, parameter
hash, config hash, ledgr and R version, dependency versions, compact
...
**Explore** (future). Fast parameter sweep mode is planned for a later
release. It will build on the same experiment object and parity
contracts, but it is not part of v0.1.7.
...
- experiment-first execution, run comparison, strategy-source
  inspection, and a deterministic demo dataset (v0.1.7).
