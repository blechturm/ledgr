...
- `ctx$targets(default = 0)`: full target vector initialized to `default`;
- `ctx$current_targets()`: full target vector initialized from current
  positions.

Use `ctx$targets()` when the strategy should go flat unless it explicitly emits
a target. Use `ctx$current_targets()` when the strategy should hold unless it
