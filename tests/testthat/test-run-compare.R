...
testthat::test_that("ledgr_compare_runs compares stored completed runs without recomputation", {
  ...
  cmp <- ledgr_compare_runs(db_path, run_ids = c("compare-qty-2", "compare-qty-1"))
...
testthat::test_that("ledgr_compare_runs compares different stored strategies", {
  ...
  cmp <- ledgr_compare_runs(db_path, run_ids = c("compare-strategy-a", "compare-strategy-b"))
...
testthat::test_that("ledgr_compare_runs counts only closing trades for win rate", {
  ...
  cmp <- ledgr_compare_runs(db_path, run_ids = "compare-roundtrip")
...
testthat::test_that("ledgr_compare_runs respects archive and incomplete-run rules", {
  ...
  on.exit(close(bt), add = TRUE)
  ledgr_run_archive(db_path, "compare-archived", reason = "test")

  testthat::expect_false("compare-archived" %in% ledgr_compare_runs(db_path)$run_id)
  testthat::expect_true("compare-archived" %in% ledgr_compare_runs(db_path, include_archived = TRUE)$run_id)
  testthat::expect_true("compare-archived" %in% ledgr_compare_runs(db_path, run_ids = "compare-archived")$run_id)
  ...
  testthat::expect_error(
    ledgr_compare_runs(db_path, run_ids = "compare-failed"),
    "Use ledgr_run_info()",
    class = "ledgr_run_not_complete"
  )
  testthat::expect_error(
    ledgr_compare_runs(db_path, run_ids = "missing-run"),
    class = "ledgr_run_not_found"
  )
  testthat::expect_error(
    ledgr_compare_runs(db_path, metrics = "experimental"),
...
testthat::test_that("ledgr_compare_runs tolerates legacy pre-provenance runs without mutation", {
  ...
  cmp <- ledgr_compare_runs(db_path, run_ids = "legacy-compare")
