...
testthat::test_that("indicator docs include compact multi-output ID references", {
  ...
  testthat::expect_match(indicators_doc, "bundle output aliases such as `bbands_dn` and `bbands_up`", fixed = TRUE)
  testthat::expect_match(indicators_doc, "Parameterized bundle concrete feature IDs may include a stable\\s+hash suffix")
  testthat::expect_match(indicators_doc, "do not call user feature factories from inside a\\s+strategy")
...
testthat::test_that("feature-map docs preserve teaching order and semantic boundaries", {
  ...
  testthat::expect_match(strategy_doc, "active-alias pattern keeps\\s+indicator-tuning inputs out of `params`")
  testthat::expect_match(strategy_doc, "ledgr_param\\(\"fast_n\"\\)")
  testthat::expect_match(strategy_doc, "ledgr_feature_grid", fixed = TRUE)
  testthat::expect_match(strategy_doc, "ledgr_strategy_grid", fixed = TRUE)
  ...
  testthat::expect_match(strategy_doc, "Use `ledgr_param()` and active aliases", fixed = TRUE)
...
testthat::test_that("sweep docs teach exploratory discipline and non-goals", {
  ...
  testthat::expect_match(doc, "Feature Grids And Strategy Grids", fixed = TRUE)
  testthat::expect_match(doc, "feature params  -> materialize parameterized features", fixed = TRUE)
  testthat::expect_match(doc, "ledgr_grid_cross", fixed = TRUE)
  testthat::expect_match(doc, "alias_map_hash", fixed = TRUE)
