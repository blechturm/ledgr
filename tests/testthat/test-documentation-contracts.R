...
testthat::test_that("metrics and accounting docs define public result semantics", {
  ...
  testthat::expect_match(metrics_doc, "Current metric assumptions are deliberately simple", fixed = TRUE)
  testthat::expect_match(metrics_doc, "default\\s+risk-free rate of `0`")
  testthat::expect_match(metrics_doc, "Annualization is inferred from the observed bar\\s+cadence")
  testthat::expect_match(metrics_doc, "no stored\\s+metric context")
  ...
  testthat::expect_match(compare_help, "default\\s+risk-free rate of")
  testthat::expect_match(compare_help, "\\code{0}", fixed = TRUE)
  testthat::expect_match(compare_help, "cadence-based annualization inferred from stored\\s+equity timestamps")
