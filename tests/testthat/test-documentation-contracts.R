...
testthat::test_that("indicator docs include compact multi-output ID references", {
  ...
  testthat::expect_match(indicators_doc, "If the factory returns a plain list, strategies read the resulting values by\\s+exact feature ID")
  testthat::expect_match(indicators_doc, "calling that external factory from\\s+inside the strategy is not Tier 1 or Tier 2")
  testthat::expect_match(indicators_doc, "use exact IDs for parameterized sweep\\s+lookups until ledgr has a first-class active-alias API")
...
testthat::test_that("helper docs state composition and whole-share target flooring", {
  ...
  testthat::expect_match(strategy_doc, "Reusable file-level helper functions are Tier 3", fixed = TRUE)
  testthat::expect_match(strategy_doc, "self_contained_strategy", fixed = TRUE)
  testthat::expect_match(strategy_doc, "`ledgr_signal_strategy()` is a separate explicit compatibility wrapper", fixed = TRUE)
  testthat::expect_match(strategy_doc, "Tier 3\\s+preflight failures remain hard failures")
  ...
  testthat::expect_match(strategy_doc, "buy_and_hold_strategy", fixed = TRUE)
  testthat::expect_match(strategy_doc, "equal_weight_strategy", fixed = TRUE)
  testthat::expect_match(strategy_doc, "single_instrument_strategy", fixed = TRUE)
  testthat::expect_match(strategy_doc, "they are not\\s+benchmark APIs")
...
testthat::test_that("feature-map docs preserve teaching order and semantic boundaries", {
  ...
  testthat::expect_match(strategy_doc, "Sweeping Indicator Parameters", fixed = TRUE)
  testthat::expect_match(strategy_doc, "exact-ID lookup from `params` is the clearest Tier 1\\s+pattern")
  testthat::expect_match(strategy_doc, "ledgr_feature_id\\(ledgr_ind_sma\\(params\\$fast_n\\)\\)")
  testthat::expect_match(strategy_doc, "Static feature maps remain the recommended shape when aliases do not vary by\\s+candidate")
  testthat::expect_match(strategy_doc, "external feature\\s+factory functions inside strategies are Tier 3")
  testthat::expect_match(strategy_doc, "active parameterized feature aliases RFC", fixed = TRUE)
...
testthat::test_that("metrics and accounting docs define public result semantics", {
  ...
  testthat::expect_match(metrics_doc, "There is also no committed `ledgr_results(bt, what = \"features\")`", fixed = TRUE)
  testthat::expect_match(metrics_doc, "`final_equity` is not a field in the `ledgr_compute_metrics()` list today", fixed = TRUE)
...
testthat::test_that("sweep docs teach exploratory discipline and non-goals", {
  ...
  testthat::expect_match(doc, "failed$params[[1]]", fixed = TRUE)
  testthat::expect_match(doc, "failed$warnings[[1]]", fixed = TRUE)
  testthat::expect_match(doc, "failed$feature_fingerprints[[1]]", fixed = TRUE)
  testthat::expect_match(doc, "failed$provenance[[1]]$feature_set_hash", fixed = TRUE)
  testthat::expect_match(doc, "promotion is only for\\s+committing a runnable candidate")
  testthat::expect_match(doc, "inherits\\(e, \"ledgr_strategy_error\"\\)")
  testthat::expect_match(doc, "feature-set hash", fixed = TRUE)
  testthat::expect_match(doc, "current bridge pattern is exact-ID lookup from `params` inside the strategy", fixed = TRUE)
  testthat::expect_match(doc, "Do not call the feature factory from inside the strategy", fixed = TRUE)
  testthat::expect_match(doc, "active-alias API gap", fixed = TRUE)
  testthat::expect_match(doc, "snapshot hash, universe, scoring range", fixed = TRUE)
  testthat::expect_match(doc, "parameter hashes", fixed = TRUE)
  testthat::expect_match(doc, "Feature-factory failures are candidate-level", fixed = TRUE)
  testthat::expect_match(doc, "execution_seed", fixed = TRUE)
  testthat::expect_match(doc, "If `seed = NULL`, row-level `execution_seed` is `NA`", fixed = TRUE)
  ...
  testthat::expect_match(doc, "The sweep table has one metric context", fixed = TRUE)
  testthat::expect_match(doc, "Same-snapshot replay is the direct way to verify", fixed = TRUE)
  testthat::expect_match(doc, "LEDGR_LAST_BAR_NO_FILL", fixed = TRUE)
  testthat::expect_match(doc, "source sweep metric context and the committed run metric context remain\\s+separate")
  ...
  testthat::expect_match(doc, "Direct CSV export of the full sweep table can fail", fixed = TRUE)
  testthat::expect_match(doc, "utils::write.csv(results_df[is_flat]", fixed = TRUE)
  testthat::expect_match(doc, "one experiment per strategy", fixed = TRUE)
...
testthat::test_that("experiment-store docs show the low-level CSV bridge", {
  ...
  testthat::expect_match(doc, "CSV/OHLC failures are local to the import and seal steps above", fixed = TRUE)
  testthat::expect_match(doc, "before `ledgr_snapshot_load\\(\\)`, `ledgr_experiment\\(\\)`, or\\s+`ledgr_run\\(\\)` enters the picture")
...
testthat::test_that("reproducibility article teaches provenance tiers and safe extraction", {
  ...
  testthat::expect_match(doc, "Captured mutable environments are also resolved", fixed = TRUE)
