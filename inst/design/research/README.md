...
| `ML_historical_data_prior_art.md` | 2026-09-25 external prior-art pass over Qlib, Zipline/Zipline-Reloaded Pipeline, QuantConnect/LEAN, Jesse, PyBroker, vectorbt and portfolioBacktest: how each exposes time x asset x feature history while preserving point-in-time semantics, with a comparative matrix, workload analysis and a lessons/avoid split. | `inst/design/rfc/rfc_point_in_time_historical_projection_v0_2_x_seed.md` (Seed v1). Pending: the seed's response stage is gated on its Section 11 probe, so no synthesis exists and the findings are not yet load-bearing. |
