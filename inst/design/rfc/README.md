...
| Asset availability / point-in-time universes / missing-data semantics | **accepted synthesis; spec packet not opened** | v0.2.x-v0.3.0; the reviewed comparative spike closed inconclusive on 2026-09-08 and selected no representation or performance winner. Seed v2 and three final-review patch sets produced the accepted synthesis on 2026-09-08. The next lifecycle step is a spec packet that resolves Section 15 and names the Section 14 gates before tickets; no implementation is open yet. Broad-equity universes with point-in-time membership and delistings remain blocked meanwhile. | `../research/Sharadar-Empirical-Evidence.md` (reviewed non-reconstructive empirical synthesis); `../research/ledgr_ragged_universe_prior_art_review.md`; `../research/rfc-evidence-handoff.md`; coordinate with `../research/Cross-Asset-Accounting-Critical-Events.md` | `rfc_asset_availability_point_in_time_universes_v0_1_9_8_seed.md` (Seed v1, historical); `rfc_asset_availability_point_in_time_universes_v0_1_9_8_response.md` (reviewed response); `rfc_asset_availability_point_in_time_universes_v0_1_9_8_response_review_addendum.md`; `rfc_asset_availability_point_in_time_universes_v0_1_9_8_spike_charter.md`; `rfc_asset_availability_point_in_time_universes_v0_1_9_8_seed_v2.md` (maintainer accepted; synthesis input); `rfc_asset_availability_point_in_time_universes_v0_1_9_8_synthesis.md` (accepted 2026-09-08; binding first-implementation direction); terminal spike report on `spike/asset-availability-pit-universes` at `d59259f`; horizon 2026-05-28, 2026-09-06, and 2026-09-08 `[data]` entries; roadmap v0.2.x ragged-universe / asset-lifetime section |
| Accounting-critical event types (now incl. time-accrual costs: interest, borrow, funding, and cross-asset non-trade economic events) | **gate without seed** | v0.2.x; the crypto-readiness spike, shorting contract RFC, and corporate-actions / instrument-master RFC must name it as a dependency when they open. | Gap 3 routing in the 2026-06-11 gaps entry; horizon 2026-06-28 `[data]` cross-asset accounting-critical events entry; `../research/Cross-Asset-Accounting-Critical-Events.md` | horizon 2026-06-11 `[strategy]` gaps entry; horizon 2026-06-28 `[data]` cross-asset entry; v0.2.x roadmap rows |
| API and representation-boundary hardening | **response v1 accepted with four maintainer decisions; response review complete, proceed to synthesis (2026-09-09)** | Proposed v0.2.0 window (the next release jumps to v0.2.0; historical RFC filenames and package/schema versions are unchanged until the packet). Decided: pro rata reversal fee allocation, fills cursor removed with its `lazy` and `stream_threshold` arguments, run handle is a durable locator, `ledgr_run_fold()` extracted in four risk-ordered stages that are extraction order only (response Sections 5 and 10). The seed author review (M-1 effect-aware phase map, two Low wording items) is patched into the response in place. Next: synthesis by a third author. | `rfc_api_representation_hardening_v0_2_0_seed.md`; `rfc_api_representation_hardening_v0_2_0_response.md`; `rfc_api_representation_hardening_v0_2_0_response_review.md`; `dev/spikes/api-representation-hardening/probe.R` and `probe_findings.md`; `../audits/v0_2_0_test_suite_audit.md` Section 7 measured evidence | horizon 2026-09-04 `[infrastructure]` post-v0.1.9.6 review entry; horizon 2026-09-04 `[docs]` all-vignette review entry; horizon 2026-09-06 `[infrastructure]` CI entry; v0.1.9.x planned roadmap row; `../spike_protocol.md` |
...
| Data cluster: PIT regressor snapshots (one unified RFC), corporate actions / instrument master, snapshot administration, snapshot lineage | parked | v0.2.x. | partial seed-shape for snapshot administration recorded in horizon; `../research/Cross-Asset-Accounting-Critical-Events.md` for corporate-actions / instrument-master and accounting-critical event context | horizon 2026-05-25/26/27 data + infrastructure entries; horizon 2026-06-28 `[data]` cross-asset entry |
| ML / cross-sectional cluster: ML-strategy architecture (leakage-safe training/refit, model-as-artifact identity, feature-engineering boundary), cross-sectional indicators, multi-strategy allocation | parked | Dedicated ML-architecture RFC at v0.2.x, gated on the temporal-CV / embargo machinery (a later validation-toolkit increment) and feature-store maturity. The 2026-06-14 seed records the architectural requirements, dependency gates, and open questions (Codex-reviewed). Multi-strategy allocation stays a separate strand (Peterson). | 2026-06-14 horizon "General ML-strategy preparedness" seed; `../research/Reproducible-Leakage-Safe-ML.md` (deep-research input, run ahead of cycle); 2026-06-09 entries; Peterson multi-strategy entry | horizon 2026-06-14 `[architecture]` + 2026-06-09 `[research]` entries |
...
| Validation toolkit (selection-integrity diagnostics + business-objective eligibility) | v0.1.9.6 shipped canonical returns, retained-return panels/projections, and native evidence-only PBO/CSCV, MinTRL, DSR, and effective-trial diagnostics after the 2026-06-14 spike gate. The 2026-06-26 amendment authorizes v0.1.9.7's full D2 all-pass eligibility layer while keeping D4 objective-filtered walk-forward identity deferred; the 2026-06-28 amendment adds a source-neutral public return-panel contract and renames diagnostics off the sweep prefix. v0.1.9.7 implements seven classed/hashable eligibility criteria plus diagnostic thresholds, opt-in closed-trade evidence, native K-Ratio, and `ledgr_sweep_filter()` as an all-candidates tear-down that cannot select or promote. `business_objective_hash` and `panel_hash` remain evidence provenance, not execution or walk-forward identity. Broader robustness criteria, scored composition, a public third-party criterion contract, per-fold train-sweep PBO, and `fold_seq` retention remain parked. | `rfc_validation_toolkit_v0_1_9_x_synthesis.md` (accepted 2026-06-12; maintainer-amended 2026-06-14, 2026-06-26, and 2026-06-28) | `rfc_validation_toolkit_v0_1_9_x_final_review.md`, `rfc_validation_toolkit_v0_1_9_x_seed_v2.md` (D1-D4 in-line), `../research/Validation-Toolkit.md`, `../ledgr_v0_1_9_6_spec_packet/`, `../ledgr_v0_1_9_7_spec_packet/`, `../ledgr_roadmap.md`, `../horizon.md` |
