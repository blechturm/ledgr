...
| API and representation-boundary hardening | **seed v1 staged, probe executed, response v1 accepted with four maintainer decisions (2026-09-09)** | Proposed v0.2.0 window (the next release jumps to v0.2.0; historical RFC filenames and package/schema versions are unchanged until the packet). Decided: pro rata reversal fee allocation, fills cursor removed, run handle is a durable locator, `ledgr_run_fold()` extracted in four risk-ordered stages (response Sections 5 and 10). Next: seed author reviews the response, then seed v2 or synthesis by a third author. | `rfc_api_representation_hardening_v0_2_0_seed.md`; `rfc_api_representation_hardening_v0_2_0_response.md`; `dev/spikes/api-representation-hardening/probe.R` and `probe_findings.md`; `../audits/v0_2_0_test_suite_audit.md` Section 7 measured evidence | horizon 2026-09-04 `[infrastructure]` post-v0.1.9.6 review entry; horizon 2026-09-04 `[docs]` all-vignette review entry; horizon 2026-09-06 `[infrastructure]` CI entry; v0.1.9.x planned roadmap row; `../spike_protocol.md` |
