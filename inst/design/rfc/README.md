...
| Validation toolkit (selection-integrity diagnostics + business-objective constructor) | v0.1.9.6 ships the validation substrate first: canonical single-run returns, retained-return panel hygiene, adapter-shaped matrix / xts projections, and reference-verified self-contained diagnostics where spec-cut keeps them. The 2026-06-14 maintainer amendment changes PBO/CSCV from unconditional implementation to an in-packet spike gate: public PBO/CSCV may ship only after a maintainer-accepted spike verifies `pbo` package/API/determinism, known-answer/reference-value behavior, ledgr panel contract, and adapter-vs-native verdict. If the gate does not pass, PBO/CSCV and the dependent `ledgr_business_objective()` / objective-filtered walk-forward identity work defer to v0.1.9.7 or later unless spec-cut records a narrowed objective-only override. Packet-open also binds the vignette styleguide's methodological-diagnostics teaching standard before validation-method tickets open. Session-object persistence only; PA extends the existing Suggests boundary; MIT core with no GPL/AGPL code transfer. Per-fold train-sweep PBO and `fold_seq` retention stay parked in horizon (2026-06-12 `[evaluation]`). | `rfc_validation_toolkit_v0_1_9_x_synthesis.md` (accepted 2026-06-12; maintainer-amended 2026-06-14) | `rfc_validation_toolkit_v0_1_9_x_final_review.md`, `rfc_validation_toolkit_v0_1_9_x_seed_v2.md` (D1-D4 in-line), `../research/Validation-Toolkit.md`, `../ledgr_roadmap.md` v0.1.9.6 row, future v0.1.9.6 packet |
