...
| API and representation-boundary hardening | parked | After the active v0.1.9.7 cycle, write a seed before another large validation or execution surface. Seed must cover API tiers, stable-export manifest, accounting/provenance invariant ownership, reversal-fee reconciliation, risk-provenance restoration, RNG temp-name hygiene, collision-proof projections, behavior-neutral file moves, `ledgr_run_fold()` phase extraction, and the API-golden-path / accessor gaps surfaced by the post-v0.1.9.6 all-vignette review. | none staged | horizon 2026-09-04 `[infrastructure]` post-v0.1.9.6 review entry; horizon 2026-09-04 `[docs]` all-vignette review entry; v0.1.9.x planned roadmap row |
