...
- Unsupported legacy paths fail or warn with classed, action-oriented
  conditions.
- `vignettes/execution-semantics.Rmd` exists and is linked from the required
  articles.
...
Batch 2 and Batch 3 may proceed in parallel after Batch 1 if the workflow
article's canonical homes are stable. Batch 4 should wait for Batch 0's legacy
boundary decision and Batch 1's workflow language. Batch 5 should wait until
the canonical homes exist.
