...
- The maintainer accepted Batch 16 on 2026-09-13 after reviewing the completed
  ...
  proportionate to the narrow production delta.
- The release playbook was read before package-gate work. Full source tests,
  the installed README, all 20 GFM renders, the 24-article pkgdown site,
  `tools::checkRd()`, Linux persistence tests, and Linux coverage passed.
- ledgr-owned DuckDB drivers request session-local extension and secret storage
  when supported. DuckDB 1.5.5 is quiet on Windows; the capability fallback
  passes without the new argument on DuckDB 1.5.2 under WSL. No warning or
  message suppression was added.
- `tools/render-vignettes-gfm.R --check --all` executes copied sources in an
  ignored temporary directory and compares normalized results with every
  committed Markdown sibling. The only normalized fields are volatile IDs and
  timestamps already generated by the examples.
- Every relative local image reference in `vignettes/*.md` is checked for a
  matching artifact. Three previously ignored figure trees are now explicitly
  tracked, and `tests/testthat/Rplots.pdf` is ignored as a generated test file.
- CRAN `pbo` is declared in `Suggests`. The unavailable-on-CRAN `quantstrat`
  cross-check moved from a permanently skipped automated test to
  `dev/manual/verify-dsr-quantstrat.R`; it remains test evidence only.
- The Linux coverage gate passed at 85.92 percent. The first two Windows covr
...
- Remote branch CI run `34784297278` then reproduced the covr shard-read
  ...
  fresh-process tests and optional mirai availability-parity test now skip
  only under covr, remain active in ordinary package checks, and a
  preserved-shard WSL coverage run passed at 85.95 percent without lowering
  the 80 percent threshold. Corrective run `34793675897` was the evidence that
  isolated the remaining mirai worker path after the fresh-process paths were
  excluded.
...
- A clean source package built in 373.5 seconds. `R CMD check` on that archive
  completed in 1023.6 seconds with `Status: OK`, including installation,
  examples, tests, and execution of all 20 vignettes.
- WSL has R 4.5.2 and DuckDB 1.5.2 but no Quarto executable. The four required
  Linux persistence tests passed there; executable documentation passed on
  Windows and remains a distinct branch-CI gate on Linux.
