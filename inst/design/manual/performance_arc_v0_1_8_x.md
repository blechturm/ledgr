...
**Anchor scope:** every `R/...:line` reference in this article resolves
against the tree at tag `v0.1.8.10`, the end of the arc it documents.
Later refactors, including the v0.2.0.0 run-coordinator extraction,
moved much of this code between files. Read these anchors as a record of
that release, not as a map of the current tree; use
`git show v0.1.8.10:<path>` to follow them.
...
## v0.1.9.x Feature Arc Performance Follow-up

The v0.1.9.1 through v0.1.9.4 feature arc added real execution-adjacent
work: cost resolver reconstruction, target-risk plan application,
saved-sweep return retention, and walk-forward orchestration over
sweep/run execution. Those features are correctness and
research-workflow capabilities first. They should not be folded into the
v0.1.8.x B2 performance story as if they were the same measurement
question.

Safe internal framing:

- v0.1.9.1 cost models add explicit timing/cost identity and fold-time
  fill-price/fee resolution; see `cost_resolver.qmd`;
- v0.1.9.3 target risk adds classed, serialized target transforms before
  fill proposals; see `target_risk_layer.qmd`;
- v0.1.9.4 walk-forward adds orchestration, persistence, and inspection
  over existing sweep/run execution; see `walk_forward_machinery.qmd`;
- any claim about net performance after these layers needs a fresh
  same-host record with the cost/risk/walk-forward surface named.

The current horizon carries a v0.1.9.6 measurement-spike candidate to
redo the peer benchmark with cost and risk chains. That would be
measurement work, not an optimization commitment and not public
marketing language.
...
- `../horizon.md` (2026-06-02 `[architecture]` B2 spot-FIFO accelerator
  scope guard)
- `../horizon.md` (2026-06-13 `[infrastructure]` peer benchmark redo
  with cost and risk chains)
- `../rfc/rfc_compiled_hot_frame_b2_v0_1_9_x_maintainer_decisions.md`
  (Decision 2 narrowing)
...
- For v0.1.9.x execution-adjacent feature traces, see
  `cost_resolver.qmd`, `target_risk_layer.qmd`, and
  `walk_forward_machinery.qmd`.
