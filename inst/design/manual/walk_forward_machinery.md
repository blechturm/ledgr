...
The walk-forward selector remains scalar-selection only. PBO/CSCV,
MinTRL, K-Ratio, DSR, and effective-trial clustering now exist as
separate selection-integrity evidence; they are not automatic
fold-selection or promotion policy. Purging/embargo, CPCV,
benchmark-relative diagnostics, all-candidate test retention, and
production promotion policy remain deferred.
...
Availability-aware folds use the sealed session calendar and the same
provider and shared fold as direct execution. An `INCOMPLETE` train
candidate is excluded from selection. An incomplete test fold marks the
session `PARTIAL` and stops a carry-state chain instead of inventing the
next opening. Completed carry-state folds derive cash from finalized
equity, positions from ledger replay, and cost basis from lot evidence.
...
- keep selection-integrity diagnostics separate from automatic fold
  selection.
