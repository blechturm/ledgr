...
shapes strict across `R/execution-spec.R:120`, `R/execution-spec.R:180`,
`R/execution-spec.R:196`, `R/execution-spec.R:220`,
`R/execution-spec.R:224`, and `R/execution-spec.R:241`.

At each pulse, `R/fold-engine.R:510` constructs the strategy context as a plain
list with class `ledgr_pulse_context` at `R/fold-engine.R:528`. The runtime
shape is:
...
choice. Fast helpers are attached at `R/fold-engine.R:530`; regular helpers are
attached at `R/fold-engine.R:543`. Both operate over the same pulse data.
...
`R/backtest-runner.R:1075`. During the fold, a strategy `state_update` is
canonicalized at `R/fold-engine.R:1025`, kept in memory at
`R/fold-engine.R:1026`, and either persisted through
`output_handler$write_strategy_state()` at `R/fold-engine.R:1028` or buffered via
`output_handler$buffer_strategy_state()` at `R/fold-engine.R:1030`.

Telemetry is a mutable environment owned by the caller. Committed runs allocate
the telemetry fields at `R/backtest-runner.R:773`; sweeps allocate the
same-style fields at `R/sweep.R:2145`. The common slots include `t_pre`,
...
persistent handler at `R/backtest-runner.R:631` with pending event and state
buffers initialized at `R/backtest-runner.R:370`. Sweep execution builds the
memory handler at `R/sweep.R:1492`; its typed event columns are allocated at
`R/sweep.R:1506` and grown with the shared event-buffer capacity function from
...
`R/run-snapshot.R:1` for the cold guard and `R/backtest-runner.R:1109` for
fold entry.

Sweeps dispatch through `ledgr_sweep()`. The public `compiled_accounting_model`
argument is normalized at `R/sweep.R:158`, carried into candidate tasks at
`R/sweep.R:317`, and passed to each candidate execution spec at
`R/sweep.R:1408`. Candidate evaluation still calls `ledgr_execute_fold()` at
`R/sweep.R:1412`.
...
`write_fill_events()` at `R/sweep.R:1909`, keep typed event metadata in memory,
and only serialize `meta_json` when callers materialize rows at
`R/sweep.R:1602`.

### Availability Path

Availability does not introduce another fold. `R/backtest-runner.R:653-680`
verifies the snapshot, constructs the internal provider, and chooses the
declared availability calendar before `ledgr_execute_fold()` receives them in
the normal execution specification. `ledgr_run_snapshot_calendar()` remains
the dense-only branch. The fold obtains each dynamic decision axis and
point-in-time view from `R/availability-provider.R`, then keeps target rules,
risk, valuation, affordability, and event emission in the shared fold.
...
Expected valuation exhaustion and unsupported terminal settlement preserve
accepted prefix economics and finalize `INCOMPLETE`. Unexpected fold errors
remain `FAILED`. Neither path fabricates an execution price or terminal cash
flow, and result inspection never invokes the strategy.
...
the committed-run guard `R/run-snapshot.R:1`, which checks `SEALED` at
`R/run-snapshot.R:32` and recomputes the hash at `R/run-snapshot.R:55`.

Strategy return-shape failures are loud inside the fold. Intermediate strategy
returns abort at `R/fold-engine.R:588`; non-list target envelopes abort at
`R/fold-engine.R:594`; full target validation runs at `R/fold-engine.R:604`.
...
The warning path is `R/fold-engine.R:113`; it preserves no-lookahead timing
...
verification (`R/run-snapshot.R:1`), bars and feature projection assembly,
cost resolver construction, output-handler construction, and execution-spec
validation (`R/execution-spec.R:120`).
...
are guarded at `R/fold-engine.R:1039`.
...
`R/sweep.R:1416`. The fast path distinction is important: optimizing result
