...
| v0.1.9.6 | Planned | Validation toolkit substrate and gated diagnostics packet (accepted 2026-06-12 synthesis, maintainer-amended 2026-06-14): canonical single-run return stream (`ledgr_results(bt, what = "returns")`), retained-return panel-hygiene bridge, adapter-shaped return matrix / xts projection, and reference-verified self-contained diagnostics where spec-cut keeps them. PBO/CSCV implementation is conditional on an in-packet PBO spike that verifies the `pbo` package/API/determinism, a known-answer or reference-value fixture, ledgr panel contract, and adapter-vs-native verdict; if the gate does not pass, PBO/CSCV and the dependent `ledgr_business_objective()` / objective-filtered walk-forward identity work defer to v0.1.9.7 or later. Packet-open also binds the methodological-diagnostics teaching standard in the vignette styleguide before validation-method tickets open. Also carries a deep intraday-readiness code audit that checks whether v0.1.x still preserves an EOD-first, intraday-tolerant architecture and estimates any refactor size; no intraday runtime implementation is authorized by the audit. First feature packet after v0.1.9.5. | Future packet; `inst/design/rfc/rfc_validation_toolkit_v0_1_9_x_synthesis.md` (accepted 2026-06-12; amended 2026-06-14); `inst/design/horizon.md` 2026-06-13 intraday-readiness audit entry |
