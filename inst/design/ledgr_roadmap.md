...
| v0.1.9.x | Planned | API and representation-boundary hardening RFC: classify the public namespace into workflow / construction / advanced-inspection / infrastructure tiers, add representation-boundary invariant tests, make provenance and retention schemas more declarative, and plan behavior-neutral file moves plus `ledgr_run_fold()` phase extraction. Local post-v0.1.9.6 findings may be fixed independently if they block active work. | Future RFC and packet; `inst/design/horizon.md` 2026-09-04 `[infrastructure]` entry |
