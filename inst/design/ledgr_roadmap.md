...
and Batches 9 through 11 are pending. Batch 10 is the final benchmark
checkpoint and Batch 11 requires an explicit maintainer go-ahead. The
...
| v0.2.0.1 | Active | Productionize the three reviewed availability seams, correct the quadratic seal validators, repair resumed-run equity-prefix finalization, linearize the two audit-proven remaining hot paths (event writes and fold-time valuation), and close with fresh separated availability and peer benchmarks. The RFC, spec, amendment, and amendment ticket cut are accepted; tickets LDG-2719 through LDG-2740 are cut, Batches 0 through 7 are complete, Batch 8 is implemented and review pending, and Batches 9 through 11 are pending. Batch 10 is the final benchmark checkpoint; Batch 11 requires explicit maintainer authorization after those results are reviewed. | `inst/design/ledgr_v0_2_0_1_spec_packet/`; accepted availability hot-path synthesis and maintainer decisions; accepted complexity amendment and audit; earlier spike inventories; snapshot-sealing probe |
