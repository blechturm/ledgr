...
| v0.1.9.7 | Active | Business-objective eligibility and validation polish: classed/hashable business-objective criteria, evidence-only all-candidates sweep filtering, closed-trade retention, strict-lattice stable-region work, conditional K-Ratio, intraday metric-context guardrail, public return-panel entry point, and Selection Integrity vignette rebuild. Automatic promotion, objective-filtered walk-forward identity, first-class intraday runtime, and paper/live work remain non-scope. | `inst/design/ledgr_v0_1_9_7_spec_packet/`; `inst/design/rfc/rfc_validation_toolkit_v0_1_9_x_synthesis.md` (2026-06-26 amendment); `inst/design/horizon.md` |
...
| v0.1.9.x | Planned | API and representation-boundary hardening RFC: classify the public namespace into workflow / construction / advanced-inspection / infrastructure tiers, add representation-boundary invariant tests, make provenance and retention schemas more declarative, and plan behavior-neutral file moves plus `ledgr_run_fold()` phase extraction. Local post-v0.1.9.6 findings may be routed through tickets and fixed independently if they block active work. | Future RFC and packet; `inst/design/horizon.md` 2026-09-04 `[infrastructure]` entry |
