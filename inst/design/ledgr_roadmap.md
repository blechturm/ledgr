...
| v0.2.0.2 | Cut 2026-09-21; cut review passed after patches; workstream 1 ready to open | Test-suite cleanup under the accepted testing-architecture RFC as the opening workstream; then equity economic events with accounting-core consolidation; fast availability results; minimal progress; consistent opening validation; first-cutoff inspection; clear unsupported-economics explanations. | This roadmap, v0.2.0.2 section; `inst/design/research/Sharadar-Corporate-Event-Evidence.md`; `inst/design/research/Sharadar-Workflow-Improvement-Evidence.md` |
| v0.2.0.2 equity corporate actions | Accepted 2026-09-24; cut 6, workstreams 10 through 12, LDG-2804 through LDG-2818; cut review accepted after two rounds; workstream 10 accepted and workstream 11 in progress | Sealed vendor-neutral `equity_corporate_actions` facts with tiered provenance; `corporate_action_policy` with a zero-configuration research preset and a strict preset entering run identity; gross cash distributions through `CASHFLOW` with entitlement fixed before the ex-dividend boundary and credited at the ex-date close; modeled terminal disposition through a new asset-neutral `DISPOSITION` event; unsupported quantity effects reported with omitted value as four reconciled quantities; headline `corporate_action_fidelity` in ordinary results; the compiled envelope refusal lands first. Not corporate-action complete: exact quantity transformation is the next row. | `inst/design/rfc/rfc_equity_settlement_post_v0_2_0_2_synthesis.md`; `inst/design/ledgr_v0_2_0_2_spec_packet/tickets.yml` cut 6; `inst/design/horizon.md` 2026-09-24 direction entry |
| Post-v0.2.0.2 exact equity quantity settlement | Scheduled for one of the next equity-accounting releases; dedicated RFC required before ticket cut | Atomically transform parent holdings into recipient holdings and cash legs, transfer model basis, handle fractions, replay and reopen exactly, and define off-axis recipient valuation. The corrected census makes this measured product work, not speculative architecture: 41 candidates, 22 daily-model-ready cases and 10 clean single-recipient stock acquisitions. v0.2.0.2 may preserve and report these facts but must not claim to settle them exactly. | Future focused RFC; `inst/design/horizon.md` 2026-09-24 exact-quantity entry; corrected Tier 2 census; equity-settlement spike record |
| Post-v0.2.0.1 governance review | Accepted 2026-09-21; pilot on the next implementation packet | Review and simplify the RFC, spike, spec, review, closeout, and testing processes while preserving independent review, evidence integrity, scope containment, explicit maintainer acceptance, honest measurement clocks, and strict semantic detection. Decide how reference-implementation authority differs from runtime defaults, and classify compiled-path guarantees without imposing a Cartesian benchmark grid. Complete this review before opening the next implementation packet. | `rfc/rfc_governance_review_post_v0_2_0_1_synthesis_v2.md`; `rfc_cycle.md` |
| Post-v0.2.0.1 accounting-core consolidation RFC | Accepted 2026-09-22; workstream to be cut, within v0.2.0.2, after the test cleanup and before equity settlement semantics and the compiled execution RFC | The inventory refuted the eight-replay premise: one canonical R lot kernel, one independent C++ implementation, six driver loops around the kernel, and one duplicated projection. The synthesis binds one kernel on a flat per-instrument lot carrier with the two per-fill scans removed, one preparer over the eleven source columns shared by memory and `ledger_events`, one prepared-event replay with projections outside, cash and positions outside the kernel, unknown operations failing closed, canonical R normative with C++ an optional fill-only counterparty, and the C++ boundary deferred to the compiled-execution RFC. Compensated accumulation and tolerance were already identical on both paths. Bound for correctness and equity landing once; the kernel is 3.5% of a shallow run. | `inst/design/rfc/rfc_accounting_core_consolidation_v0_2_0_2_synthesis.md`; `inst/design/audits/post_v0_2_0_1_accounting_core_inventory.md`; `inst/design/horizon.md` 2026-09-22 entry |
| Post-v0.2.0.1 ingestion consolidation | Done 2026-09-22, v0.2.0.2 cut 3 workstream 7 (`bfb2304` through `c43fe1a`) | Maintainer product decision: `ledgr_snapshot_from_csv()` and `ledgr_snapshot_from_df()` are the ingestion surface, and the connection-first create/import/seal lifecycle from v0.1.1 is removed. The two CSV surfaces were two generations of one idea: the strict importer came first, the handle-returning adapters re-implemented its four validations in a second error vocabulary two days later, and every capability added since, availability facts and quarantine included, landed only on the adapters. `from_csv()` gains the instruments file, facts and quarantine arguments; `LEDGR_CSV_FORMAT_ERROR` is gone; timestamp normalization is exact and deduplicated, with the snapshot hash pinned as LCL-0015. No RFC: the 2026-09-17 durable-path note reserved this call for the maintainer. | `inst/design/horizon.md` 2026-09-22 `[product] One CSV ingestion surface`; `inst/design/ledgr_v0_2_0_2_spec_packet/ingestion_consolidation_closeout.md`; `ws7_migration_evidence.md`; `dev/bench/notes/durable_path_observations.md` decisions 1 and 2 |
| Post-v0.2.0.1 ingestion reader | Done 2026-09-22, v0.2.0.2 cut 3 workstream 9, which ran as cut 5 and was folded in | Maintainer product decision: `duckdb::read_csv_auto()` replaces `utils::read.csv()` in the ingestion reader, with `instrument_id`, `ts_utc` and `symbol` forced to text so ledgr keeps owning instrument identity and every accepted timestamp form. duckdb was already an Import. The read goes from 4.91 s to 0.26 s at the release shape and `ledgr_snapshot_from_csv()` from 10.72 s to 7.23 s, but the decisive finding was a correctness defect: a leading-zero instrument id was silently sealed as an integer, so affected snapshots carry wrong identifiers and wrong hashes. A compatibility matrix registered as a test before the swap made each change declare itself, but only for the shapes it enumerated: two close reviews found three more behaviours in its gaps, and a fourth, that a quarantined row's saved copy put ignored columns into snapshot identity, was older than this work. An enumerated witness proves its enumeration, not its surface. | `inst/design/horizon.md` 2026-09-22 `[product] DuckDB reads the ingestion CSV`; `inst/design/ledgr_v0_2_0_2_spec_packet/ingestion_reader_closeout.md`; OPT-L03 |
...
| Post-v0.2.0.2 design-corpus split | Scheduled for the version after v0.2.0.2; not in this cycle | `horizon.md` is being asked to be two things with opposite lifecycles and the parking lot is losing. A parking lot must be cheap to add to and cheap to throw away; an authority record must never be deleted. Measured 2026-09-22: 8,467 lines, 142 dated entries, 120 files citing it, 122 of the 142 entries cited somewhere, 62 citations using binding language ("bound by", "governed by", "per the horizon entry"), and only 20 entries, 546 lines, cited by nothing. The top 20 entries hold 38 percent of the file. The drift happened because horizon is the only design file with no admission rule, so every observation that did not clear the bar for an RFC, an ADR, `contracts.md` or a spec packet landed there, mixing four kinds of thing: speculative ideas, measured findings, refuted premises and binding scope guards. Outcome wanted: horizon is the parking lot only, under one enforceable invariant, that no other document may cite it. A citation is the signal that an idea has graduated and must move. Findings go to `dev/bench/notes/` or `inst/design/audits/`; binding scope guards go to `contracts.md` or an ADR, the B2 spot-FIFO guard being the clearest case since it is named as the binding constraint for a deprecated ADR while living in a parking lot; RFC deferrals go into the RFC synthesis or the packet closeout. Method, so the migration is not one large pass: write the invariant into the header first; move only the entries cited with binding language; compress settled entries in place, keeping their headers so existing anchors resolve, rather than deleting them; delete only the uncited speculative set, and only after the maintainer reads the titles. No RFC: this is a documentation restructure with no product or contract consequence, but it touches roughly 120 citing files, so it needs its own cycle rather than being done alongside implementation work. | This roadmap row; `inst/design/horizon.md` header; `inst/design/rfc/README.md` Topic Decision Index, which carries the binding-authority citations; `inst/design/contracts.md`; `inst/design/adr/README.md` |
...
| --- | --- | --- |
| LFB-001/011 equity events | In this release, evidenced gross cash distributions and explicit modeled terminal disposition, with strict refusal, identified assumptions and unavoidable reporting. Preserve normalized security, mixed-acquisition and spin-off facts, but do not mutate quantities or transfer basis yet. | Equity-corporate-action RFC after accounting-core consolidation. Exact atomic parent/recipient transformations are scheduled for a subsequent equity-accounting release and require their own RFC. |
...
Maintainer scope decision, 2026-09-24: v0.2.0.2 is not the exact quantity
settlement release. Its product claim is bounded to cash effects, modeled
terminal disposition, strict refusal and visible unsupported economics. The
follow-up exact-quantity RFC owns stock-for-stock, mixed and spin-off position
mutation, atomic group persistence, model-basis transfer, fractions and
off-axis recipient valuation. This is scheduled product work rather than an
unowned deferral; the release may not describe itself as corporate-action
complete before that work lands.
...
**Resolved 2026-09-21.** The review's synthesis
(`rfc/rfc_governance_review_post_v0_2_0_1_synthesis_v2.md`) is accepted.
Implementation authority: canonical R is the normative executable oracle for
optimized and compiled paths; runtime default is a separate decision; an
intentional divergence needs its own RFC and fails closed until accepted;
contract-level defects are resolved before canonical R is corrected. The
review model — maintainer-owned briefs, Type 1 and Type 2 modes, workstream
review, one ticket source, five routes — binds as a pilot on the next
implementation packet with a preregistered gate of at most 0.5 review
invocations per completed ticket, and becomes the default only after the
maintainer reviews that packet's closeout. The test-suite audit runs as an
audit route with its own Type 2 brief and feeds a testing-architecture RFC,
which decides lanes, CRAN strategy, the canonical-R differential lane's
