...
$stdout = Join-Path $env:TEMP ("ledgr_w8_peer_" + [guid]::NewGuid() + ".out")
$stderr = Join-Path $env:TEMP ("ledgr_w8_peer_" + [guid]::NewGuid() + ".err")
$sampleTemp = Join-Path $env:TEMP ("ledgr_w8_peer_" + [guid]::NewGuid() + ".csv")
...
$summary = [pscustomobject]@{
  ...
  source_commit = "9cb8c2e692c821cd7fb57831fc81865ffc96b080"
...
if ($null -ne $process.ExitCode -and $process.ExitCode -ne 0) {
  throw "Workstream 8 peer benchmark exited with code $($process.ExitCode)."
}

& $Rscript `
  "dev/bench/v0_2_0_2_workstream8/write_quantstrat_environment.R" `
  $stem
